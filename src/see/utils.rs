...
pub fn parse_rhs<'a, 'b>(
    ...
    match rhs {
        ...
        Rhs::OOXExpression(expr) => Ok(Some(Expression::substitute(&sym_memory, expr.clone()))),
...
pub fn assert(
    ...
    solver: &mut SolverEnv,
    assertion: &OOXExpression,
) -> Result<(), CounterExample> {
    let mut sym_assertion = Expression::substitute(&sym_memory, assertion.clone());
    ...
    match solver.satisfiable(&Expression::Not(Box::new(constraints)), sym_memory, i) {
        Some(model) => {
...
pub fn assume(
    ...
    solver: &mut SolverEnv,
    assumption: &OOXExpression,
) -> (PruneProbability, bool) {
    let mut sym_assumption = Expression::substitute(&sym_memory, assumption.clone());
    let config = &solver.config;
    ...
    if config.expression_evaluation {
        ...
        match simple_assumption {
            Expression::Literal(Literal::Boolean(false)) => return (prune_p, false),
            Expression::Literal(Literal::Boolean(true)) => (),
    ...
    match &constraints {
        ...
        Expression::Literal(Literal::Boolean(false))=> (prune_p, false),
        Expression::Literal(Literal::Boolean(true)) => (prune_p, true),
        // if p > random number between 0..100 then attempt to prune path and update p
        _ if p >= rand::thread_rng().gen_range(1..=100) => {
            match solver.satisfiable(&constraints, sym_memory, i){
